...
再看下边界条件，长度为 0 或 1 的字符串可以作为回文字符串返回，长度 2 以上需要做判断。
...
又因为 (i ... j) 区间内部的回文，我们不需要知道具体内容，只需要一个判断的结果，在循环外加上一个二维数组存入 i、j 坐标，及其判断结果，下次循环直接利用存入的判断，减少重复判断
